      ...
      <div class="cytext">
        ...
        <div class="cyuu">
          ...
          <p class="cybtn">
            ...
            <span v-on:click="clickItemGo()" style="font-size:19px">带我去</span>
          </p>
        </div>
      </div>
    </div>
    <gytUtilityQR :itemGo="content" v-if="showlocation" @fireClose="afterItemGoClose()"></gytUtilityQR>
  </div>
</template>
...
export default {
  ...
  mounted: function () {
    let self = this
    self.content.img = self.itemInfo.img
    self.content.startlatLng = startlatLng
    self.$api.ask(self.itemInfo.text, (res) => {
      console.log(res)
      switch (res.type) {
        case 0:
        case '500':
