<!--<template xmlns:v-el="http://www.w3.org/1999/xhtml">-->
  <!--<div class="show-notice">-->
    <!--<div class="notice-list" :style="{transform: 'translateY(-'+noticePosition+'px) translateZ(0px)'}">-->
      <!--<p v-for="notice in notices" track-by="$index">{{notice}}</p> </div> </div>-->
...
<!--<script>-->
    <!--export default {-->
        <!--name: "test",-->
      <!--data () { return { noticePosition: 0  } },-->
       <!--compiled () { let destination = 30-->
         <!--setInterval(() => {-->
         ...
         <!--}else { // 列表到底-->
           <!--this.noticePosition = 0 // 设置列表为开始位置-->
             ...
             <!--this.move(destination, 500)-->
             <!--destination += 30-->
           <!--} }, 2500) }, methods: { move (destination, duration) { // 实现滚动动-->

           <!--let speed = ((destination - this.noticePosition) * 1000) / (duration * 60)-->
            <!--let count = 0-->
          <!--let step = () => { this.noticePosition += speed-->
            <!--count++-->
...
<!--<style lang="scss" scoped>-->
  <!--$notice-height: 30px;-->
  <!--.show-notice{ height: $notice-height; overflow: hidden; vertical-align: middle; .notice-list p{ margin: 0; height: $notice-height; line-height: $notice-height; } }-->
